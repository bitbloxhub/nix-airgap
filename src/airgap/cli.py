...
async def probe_batch(paths: list[str], caches: list[str]) -> dict[str, str | None]:
	limits = httpx.Limits(
...
async def add_fods(
	...
	async def add_fod(fod: str) -> None:
		...
		if added != fod:
			raise RuntimeError(f"FOD path mismatch: expected {fod}, got {added}")

	await asyncio.gather(*(add_fod(fod) for fod in fods))


def fod_metadata(fods: list[str]) -> dict[str, dict[str, object] | None]:
	if not fods:
		return {}
	return cast(
		dict[str, dict[str, object] | None],
		json.loads(run("nix", "path-info", "--json", "--json-format", "1", *fods)),
	)
...
def split_fods_by_metadata(
	...
	if missing:
		raise RuntimeError("FOD metadata missing after local realization: " + ", ".join(missing))
	copyable = [
		fod for fod, info in metadata.items() if info is not None and info.get("ca") is not None
	]
	broken = [fod for fod, info in metadata.items() if info is not None and info.get("ca") is None]
...
def _main() -> None:
	...
	with tempfile.TemporaryDirectory(prefix="nix-airgap-") as tmp_name:
		...
		print("==> Evaluating")
		derivations = json.loads(run("nix", "derivation", "show", "-r", args.installable))[
			"derivations"
		]
		drv = (
			args.installable
		...
		cache_frontier = unique(
			[f"{cache}\t{path}" for kind, cache, path in plan if kind == "cache"]
		)
		...
		if fods:
			...
			if args.show_fod_frontier:
				...
				for fod in fods:
					info = fod_info[fod]
					status = (
						"valid ca" if info is not None and info["ca"] is not None else "missing ca"
					)
		...
		for cache in unique([line.split("\t", 1)[0] for line in cache_frontier]):
			paths = [
				line.split("\t", 1)[1] for line in cache_frontier if line.startswith(f"{cache}\t")
			]
			if paths:
